...
def executeODES(images_directory, generate_annotated_images):
    ...
    if generate_annotated_images:
        annotated_images_directory = os.path.join(script_directory, images_directory, "annotated_images")
        os.makedirs(annotated_images_directory, exist_ok=True)
        thumbnails_directory = os.path.join(script_directory, images_directory, "annotated_images", "thumbnails")
    ...
    for image_name in sorted(os.listdir(images_directory)):  # Added sorting here for file listing
        if image_name.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            ...
            for result in result_generator:
                ...
                stage_classification, total_cells, leuko_perc, corni_perc, nucl_perc, is_low_cell_count = determine_estrus_stage(
                    result.boxes.data, model.names)
                ...
                if generate_annotated_images:
                    ...
                    cv2.imwrite(annotated_images_directory + "/thumbnails/" + f"t-annotated_{image_name}-120.jpg", im)
    ...
    output_csv_path = os.path.join(script_directory, images_directory, 'results.csv')
    counter = 1
...
def box_label(image, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
    ...
    if label:
        ...
        cv2.putText(image, label, (p1[0], p1[1] - 2 if outside else p1[1] + h + 2), 0, lw / 6, txt_color, thickness=tf,
